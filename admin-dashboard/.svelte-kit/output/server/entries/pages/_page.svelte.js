...
function toFont(options, fallback2) {
  ...
  let size = valueOrDefault(options.size, fallback2.size);
  if (typeof size === "string") {
    size = parseInt(size, 10);
  }
  let style = valueOrDefault(options.style, fallback2.style);
  ...
  const font = {
    ...
    lineHeight: toLineHeight(valueOrDefault(options.lineHeight, fallback2.lineHeight), size),
    size,
    style,
...
function _createResolver(scopes, prefixes = [
  ""
], rootScopes, fallback2, getTarget = () => scopes[0]) {
  const finalRootScopes = rootScopes || scopes;
  if (typeof fallback2 === "undefined") {
  ...
  const cache = {
    ...
    override: (scope) => _createResolver([
      scope,
      ...scopes
    ], prefixes, finalRootScopes, fallback2)
  };
...
function addScopes(set2, parentScopes, key, parentFallback, value) {
  for (const parent of parentScopes) {
    ...
    if (scope) {
      ...
      if (typeof fallback2 !== "undefined" && fallback2 !== key && fallback2 !== parentFallback) {
        return fallback2;
      }
...
function createSubResolver(parentScopes, resolver, prop, value) {
  ...
  if (typeof fallback2 !== "undefined" && fallback2 !== prop) {
  ...
  return _createResolver(Array.from(set2), [
    ""
  ], rootScopes, fallback2, () => subGetTarget(resolver, prop, value));
}
...
function MetricTile($$payload, $$props) {
  ...
  let color2 = fallback($$props["color"], "blue");
  ...
  bind_props($$props, { title, value, unit, trend, icon, color: color2 });
  pop();
}
