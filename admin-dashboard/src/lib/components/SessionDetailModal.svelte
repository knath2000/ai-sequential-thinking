<script lang="ts">
  ...
  export let show: boolean; // Add show prop for bind:show
...
{#if show && sessionDetail}
  ...
  >
    <div
      class="modal-content bg-white dark:bg-gray-900 rounded-lg shadow-lg w-11/12 max-w-3xl p-4 max-h-[90vh] overflow-auto"
    ...
    >
      ...
      <h2 class="text-lg font-semibold mb-2 text-gray-900 dark:text-white">
        Session {sessionDetail.session.session_id}
      </h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <pre
            class="text-xs font-mono bg-gray-100 dark:bg-gray-800 p-2 rounded mt-2 overflow-auto max-h-32 text-gray-800 dark:text-gray-200"
          >
      ...
      <div class="mb-4">
        ...
        <div
          class="mt-2 max-h-48 overflow-auto text-xs font-mono bg-gray-50 dark:bg-gray-800 p-2 rounded text-gray-800 dark:text-gray-200"
        >
            ...
            <ul class="space-y-1">
              {#each sessionDetail.events as e}
                <li class="py-1">
                  ...
                  {e.tool_name || ''} — {e.response_time_ms ?? '-'}ms
                </li>
              {/each}
            </ul>
          {/if}
          ...
          {:else}
            <ul class="space-y-1">
              {#each sessionDetail.logs as l}
                <li class="py-1">
                  {new Date(l.timestamp).toLocaleString()} [{l.level}] —
                  {l.error_type}: {l.error_message}
                </li>
              {/each}
            </ul>
          {/if}
